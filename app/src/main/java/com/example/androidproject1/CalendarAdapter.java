...
public class CalendarAdapter extends BaseAdapter {
    ...
    // 초기화 메소드 설정
    // 1 개월의 일별 데이터를 담고 있을 수 있는 MonthItem 의 배열 객체 생성
    // 원하는 날짜 지정
    // 현재 달은 4 월 이므로 3 월입력
    ...
    // 다음 월로 이동 시 MONTH +1 // recalculate(), resetDayNumbers() 사용
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // ConvertView 는 안드로이드가 사용자의 편의성을 위해 제공하는 객체
    ...
    // 한 달이 31 일, 30 일, 혹은 2 월 달의 윤년을 계산하기 위한 메소드
    private int getMonthLastDay(int year, int month) {
        switch (month) {
            ...
            case 9:
            case 11:
                return (31); //한 달 31 일 리턴
            case 3:
            case 5:
            case 8:
            case 10:
                return (30); //한 달 30 일 리턴
            default:
                if( ( (year%4 == 0) && (year%100 != 0) ) || (year%400 == 0) ) { //윤년 계산
                    return (29); //한 달 29 일 리턴
                } else {
                    return (28); //한 달 28 일 리턴
                }
        }
    }
}